...
class CaptioningRNN(object):
  ...
  def sample(self, features, max_length=30):
    ...
    start = np.array([[self._start]] * N)
    for i in range(max_length):
        (x_t, embed_cache) = word_embedding_forward(start, W_embed)
        x_t = np.transpose(x_t, [1, 0, 2])
        if self.cell_type == 'lstm':
            (h, c0, rnn_cache) = lstm_step_forward(x_t[-1], h0, c0, Wx, Wh, b)
        else:
            (h, rnn_cache) = rnn_step_forward(x_t[-1], h0, Wx, Wh, b)
        h_t = h.reshape([1, h.shape[0], h.shape[1]])
        h_t = np.transpose(h_t, [1, 0, 2])
        scores = np.dot(h, W_vocab) + b_vocab
        captions[:, i] = np.argmax(scores, axis=1)
        start = np.array([captions[:, i]]).T
        h0 = h
