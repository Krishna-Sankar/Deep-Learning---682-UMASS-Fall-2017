...
class KNearestNeighbor:
  ...
  def compute_distances_no_loops(self, X):
    ...
    #########################################################################
    vector_11 = np.dot(X, self.X_train.T)
    vector_train = np.square(self.X_train).sum(axis = 1)
    vector_test = np.square(X).sum(axis = 1)
    temp_1=np.matrix(vector_test).T
    temp_2=vector_train+temp_1+vector_11*(-2)
    dists = np.sqrt(temp_2)
    return dists
  ...
  def predict_labels(self, dists, k=1):
    ...
    for i in xrange(num_test):
      ...
      # label.                                                                #
      #########################################################################
      y_pred[i] = np.argmax(np.bincount(temp))
    return y_pred
